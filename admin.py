...
def menu():
    ...
    print("9. Delete an account")
    print("10. Get all insights of your bank")
    print("0. Quit")
...
def bc6():
    ...
    tenure=int(input("Enter your job tenure : "))
    loanamount=int(input("How much amount you want as loan"))
...
def bc9():
    con,cur=initialeverytime()
    accno=input("Enter the account number of the account to be deleted : ")
    query="DELETE FROM bank_customer WHERE Accountno="+accno
    cur.execute(query)
    con.commit()

    accno="bc"+str(accno)
    query="DROP TABLE "+accno
    cur.execute(query)
    con.commit()
...
def bc10():
    ...
    a=0
    b=0
    cur.execute("SELECT Amount FROM bank_customer")
    for i in cur:
        x=x+int(i[0])
        y=y+1

    print("Total amount in your bank : ",x)
    print("Total number of account holders : ",y)
    
    cur.execute("SELECT MAX(Amount) FROM bank_customer")
    for i in cur:
        a=i
    print("Bank depositor with highest balance : ",a)
    cur.execute("SELECT MIN(Amount) FROM bank_customer")
    for i in cur:
        b=i
    print("Bank depositor with lowest balance : ",b)
    z=x/y
    print("Average balance per depositor : ",z)
    redirecttomenu()
